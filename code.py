...
# ✅ structured output (required for routing to work)
class IntentOutput(BaseModel):
...
# ✅ IMPORTANT: use structured llm
intent_llm = gllm.with_structured_output(IntentOutput)


# ✅ IMPORTANT: create agent with structured LLM
...
def intent_node(state: Context):
    ...
    # ✅ structured object → dict (for debugging / storage)
    state.intent_data = result.model_dump()

    # ✅ CRITICAL: sync into routing fields
...
def conditional_edge(state: Context) -> Literal[
    ...
    intent = state.intent
    confidence = state.intent_confidence or 0.7
...
def agents(user_input: str, thr_id: str, usr_id: str):
    ...
    result = insurance_chain.invoke(
        input_state,{"recursion_limit": 10}
    )
